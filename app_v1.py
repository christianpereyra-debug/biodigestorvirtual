...
def mostrar_animacion_biodigestor(volumen_m3, temp_c, modo_agitacion):
    # Definir color del indicador según la temperatura
    if temp_c < 30:
        color_temp = "#3498db"  # Azul (Frío / Subóptimo)
    elif temp_c <= 38:
        color_temp = "#2ecc71"  # Verde (Óptimo Mesofílico)
    else:
        color_temp = "#e74c3c"  # Rojo (Cálido / Termofílico)
    ...
    html_code = f"""
    <div style="display: flex; justify-content: center; align-items: center; background-color: #f8f9fa; padding: 15px; border-radius: 12px; border: 1px solid #e0e0e0;">
        <svg width="340" height="250" viewBox="0 0 340 250">
            <defs>
                <linearGradient id="gasGrad" x1="0%" y1="0%" x2="0%" y2="100%">
                    ...
                    <stop offset="100%" stop-color="#f1c40f" stop-opacity="0.45"/>
                </linearGradient>
                <linearGradient id="digGrad" x1="0%" y1="0%" x2="0%" y2="100%">
                    <stop offset="0%" stop-color="#795548"/>
                    <stop offset="100%" stop-color="#4e342e"/>
                </linearGradient>
            </defs>

            <!-- Cúpula / Gasómetro con efecto de pulsación -->
            <path d="M 70 110 Q 170 20 270 110 Z" fill="url(#gasGrad)" stroke="#e67e22" stroke-width="2">
                <animate attributeName="d" 
                         values="M 70 110 Q 170 20 270 110 Z; M 70 110 Q 170 10 270 110 Z; M 70 110 Q 170 20 270 110 Z" 
                         dur="3s" repeatCount="indefinite" />
            </path>
            <text x="170" y="65" text-anchor="middle" fill="#d35400" font-weight="bold" font-size="12">Biogás (CH₄ + CO₂)</text>

            <!-- Burbujas de Gas Ascendentes -->
            <circle cx="130" cy="170" r="3" fill="#f39c12" opacity="0.7">
                <animate attributeName="cy" values="170;110" dur="2s" repeatCount="indefinite"/>
                <animate attributeName="opacity" values="0.8;0" dur="2s" repeatCount="indefinite"/>
            </circle>
            <circle cx="210" cy="180" r="4" fill="#f39c12" opacity="0.7">
                <animate attributeName="cy" values="180;105" dur="2.8s" repeatCount="indefinite"/>
                <animate attributeName="opacity" values="0.8;0" dur="2.8s" repeatCount="indefinite"/>
            </circle>

            <!-- Cuerpo del Tanque Digestor -->
            <rect x="70" y="110" width="200" height="120" rx="6" fill="none" stroke="#34495e" stroke-width="4" />
            
            <!-- Nivel de Digestato / Mezcla Orgánica -->
            <rect x="74" y="130" width="192" height="96" rx="4" fill="url(#digGrad)" />

            <!-- Agitador Mecánico Central -->
            <line x1="170" y1="90" x2="170" y2="190" stroke="#2c3e50" stroke-width="4" />
            <g transform="translate(170, 170)">
                <g>
                    <line x1="-35" y1="0" x2="35" y2="0" stroke="#ecf0f1" stroke-width="5" stroke-linecap="round"/>
                    <line x1="-15" y1="-8" x2="-15" y2="8" stroke="#ecf0f1" stroke-width="3" stroke-linecap="round"/>
                    <line x1="15" y1="-8" x2="15" y2="8" stroke="#ecf0f1" stroke-width="3" stroke-linecap="round"/>
                    {'<animateTransform attributeName="transform" type="rotate" from="0" to="360" dur="' + duracion_agit + '" repeatCount="' + anim_agit + '" />' if modo_agitacion != "sin_agitacion" else ''}
                </g>
            </g>

            <!-- Tubería Entrada Sustrato -->
            <path d="M 20 145 L 74 155" stroke="#8d6e63" stroke-width="8" stroke-linecap="round" />
            <text x="22" y="135" fill="#5d4037" font-size="11" font-weight="bold">Entrada Sustrato</text>

            <!-- Tubería Salida Biogás -->
            <path d="M 170 42 L 170 18 L 290 18" stroke="#f39c12" stroke-width="5" fill="none" />
            <text x="230" y="33" fill="#e67e22" font-size="11" font-weight="bold">Salida Biogás</text>

            <!-- Badge de Temperatura Operativa -->
            <rect x="220" y="195" width="45" height="22" rx="4" fill="{color_temp}" />
            <text x="242" y="210" text-anchor="middle" fill="white" font-size="11" font-weight="bold">{temp_c}°C</text>
        </svg>
    </div>
    """
...
if es_avanzado:
    ...
    # Ajustes estándar automáticos para el Modo Simple
...
# --- ESQUEMA ANIMADO Y VISTAS ---
st.subheader("🖼️ Esquema de Funcionamiento del Biodigestor")
mostrar_animacion_biodigestor(vol_digestor, temperatura, modo_agit)
st.caption(f"📏 **Volumen estimado del tanque:** {vol_digestor:.1f} m³ | 🌡️ **Temperatura de proceso:** {temperatura}°C")
st.divider()
